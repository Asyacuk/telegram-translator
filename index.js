...
bot.start((ctx) => {
    ctx.reply(`<b>Merhaba @${ctx.from.username}, bana çevirmek istediğin bir metin gönder senin için türkçeye çevireyim.</b>`, {parse_mode: 'HTML'});
    });
...
bot.on('text', async (ctx) => {
    ...
    ctx.reply(`*${translation.text}*`, {parse_mode: 'Markdown',
...
bot.on('photo', async (ctx) => {
    ...
    if(text) {
        ...
        ctx.replyWithPhoto(ctx.update.message.photo[0].file_id, { 
            ...
            parse_mode: 'Markdown', reply_to_message_id: ctx.update.message.message_id
        });
